...
module.exports = {
  create: async function (req, res) {
    ...
    sails.log(`Created the Assets(s) successfully: ${JSON.stringify(createdAssets)}`);
  ...
  get: async function (req, res) {
    ...
    for(let asset of assets){
      {asset.asset_type === 'Software' ? (
        ...
        asset.assetName = asset.hardware_type
      )}
  ...
  getById: async function (req, res) {
    ...
    for(let consumer of asset.consumers){
      let consumerRecord = await User.findOne({where: {id:consumer.consumer_id},select:['first_name','last_name']});
    ...
    let ownerRecord = await User.findOne({where: {id:asset.owner_id},select:['first_name','last_name']});
    asset.ownerName = ownerRecord.first_name+' '+ownerRecord.last_name;
    let productRecord = await Product.findOne({where: {id: asset.product_id}, select: ['display_name']});
    asset.productName = productRecord.display_name;
  ...
  update: async function (req, res) {
    ...
    if (updatedAsset) {
      sails.log(`Updated the Asset of id ${id} with ${JSON.stringify(payload)}`);
    } else {
